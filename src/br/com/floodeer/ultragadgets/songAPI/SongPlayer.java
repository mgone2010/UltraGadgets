...
public abstract class SongPlayer {
	...
	protected void calculateFade() {
		...
		double targetVolume = Interpolator.interpLinear(new double[] { 0, fadeStart, fadeDuration, fadeTarget },
				fadeDone);
	...
	public void addPlayer(Player p) {
		synchronized (this) {
			if (!playerList.contains(p.getName())) {
				...
				ArrayList<SongPlayer> songs = NoteBlockPlayerMain.playingSongs.get(p.getName());
				if (songs == null) {
	...
	public void removePlayer(Player p) {
		synchronized (this) {
			...
			if (NoteBlockPlayerMain.playingSongs.get(p.getName()) == null) {
				return;
			}
			ArrayList<SongPlayer> songs = new ArrayList<SongPlayer>(NoteBlockPlayerMain.playingSongs.get(p.getName()));
			songs.remove(this);
			NoteBlockPlayerMain.playingSongs.put(p.getName(), songs);
